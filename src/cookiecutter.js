...
module.exports = function run(configLocation, outputBase = process.env.PWD, templateBase = path.dirname(configLocation)) {
    ...
    .then(config => {
        return renderFiles(config, configLocation, outputBase, templateBase);
    })
    .catch(e => {
        log.error(e);
